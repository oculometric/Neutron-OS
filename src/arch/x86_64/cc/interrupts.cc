...
void initIDT(uint16_t gdt64Code) {
   ...
   idt_set_gate(0, (uint64_t)isr0 , 0x01, 0x8E);
   idt_set_gate(1, (uint64_t)isr1 , 0x01, 0x8E);
	 for (int i = 2; i < 256; i++) {
		 if (i != 31) {
			 idt_set_gate(i, (uint64_t)isr8 , 0x01, 0x8E);
		 }
	 }

   idt_set_gate(31, (uint64_t)isr31, 0x01, 0x8E);
